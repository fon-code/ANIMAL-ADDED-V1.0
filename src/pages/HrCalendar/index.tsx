...
export default function HrCalendar() {
  ...
  const [events, setEvents] = useState([
    ...
    // Mock Data
    { id: 'EV-001', date: '2026-03-09', title: 'Mixing: Vitamin Pre-mix A', time: '09:00', type: 'Production', priority: 'High', status: 'Scheduled', color: 'bg-blue-50 text-blue-700 border-blue-100', isHoliday: false },
    { id: 'EV-002', date: '2026-03-12', title: 'Quality Control: Batch 240B', time: '13:00', type: 'QA/QC', priority: 'Critical', status: 'Confirmed', color: 'bg-emerald-50 text-emerald-700 border-emerald-100', isHoliday: false },
    { id: 'EV-003', date: '2026-03-15', title: 'Maintenance: Line 3', time: '10:00', type: 'Maintenance', priority: 'Normal', status: 'Scheduled', color: 'bg-amber-50 text-amber-700 border-amber-100', isHoliday: false },
    { id: 'EV-004', date: '2026-03-18', title: 'Supply Delivery: Minerals', time: '08:00', type: 'Logistics', priority: 'High', status: 'Confirmed', color: 'bg-slate-100 text-slate-700 border-slate-200', isHoliday: false },
    { id: 'EV-005', date: '2026-03-20', title: 'Safety Audit', time: '15:30', type: 'Audit', priority: 'Normal', status: 'Confirmed', color: 'bg-purple-50 text-purple-700 border-purple-100', isHoliday: false },
  ]);
  ...
  const handleSaveEvent = () => {
    ...
    const typeColors: Record<string, string> = {
      ...
      'QA/QC': 'bg-emerald-50 text-emerald-700 border-emerald-100',
  ...
  return (
    <div className="flex flex-col h-full animate-fadeIn w-full relative">
      ...
      {/* Header */}
      <header className="flex flex-wrap items-center justify-between py-2 gap-4 shrink-0 mb-4 bg-transparent pb-4">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h2 className="text-[#141A26] font-black tracking-tighter text-2xl uppercase leading-none">
                    FACTORY <span className="text-[#D91604]">CALENDAR</span>
                </h2>
                <p className="text-[#4F868C] text-[10px] font-bold mt-1 uppercase tracking-widest leading-none">Production & Operations Planning</p>
            </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500 shrink-0">
            <KpiCard title="Production" val={filteredEvents.filter(e=>e.type==='Production').length} color={palette.teal} IconComponent={Users} desc="Active Batches" />
            <KpiCard title="Upcoming Holidays" val={events.filter(e=>e.isHoliday && new Date(e.date) >= new Date()).length} color={palette.accent} IconComponent={Palmtree} desc="Plant Shutdown" />
            <KpiCard title="Maintenance" val={events.filter(e=>e.type==='Maintenance').length} color={palette.gold} IconComponent={Settings} desc="Line Checks" />
            ...
            <div className="bg-[#F2F0EB] w-full max-w-xl rounded-[32px] overflow-hidden shadow-2xl border-t-[8px] border-[#F2B705] flex flex-col animate-in zoom-in-95 duration-300">
                ...
                <div className="p-8 space-y-6 bg-white flex-1 overflow-y-auto custom-scrollbar font-sans">
                    <div className="space-y-1.5">
                        ...
                        <input value={eventForm.title} onChange={e=>setEventForm({...eventForm, title: e.target.value})} className="w-full border-b-2 border-[#4F868C]/20 bg-transparent py-2 text-sm font-bold text-[#141A26] focus:border-[#F2B705] outline-none transition-colors" placeholder={eventForm.isHoliday ? "ระบุชื่อวันสำคัญ..." : "Mixing, QA/QC, Maintenance..."} />
                    </div>
                      ...
                      <div className="grid grid-cols-3 gap-6">
                          <div className="space-y-1.5">
                              ...
                              <select value={eventForm.type} onChange={e=>setEventForm({...eventForm, type: e.target.value})} className="w-full border-b-2 border-[#4F868C]/20 bg-transparent py-1.5 text-xs font-bold text-[#141A26] outline-none cursor-pointer focus:border-[#F2B705] transition-colors">
                                <option>Production</option><option>QA/QC</option><option>Maintenance</option><option>Logistics</option><option>Audit</option>
                              </select>
                          </div>
                          <div className="space-y-1.5">
        ...
        <div className="fixed inset-0 z-[500] flex justify-end animate-in fade-in duration-300">
            ...
            <div className="relative w-full max-w-md bg-[#F2F0EB] h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 font-sans">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 text-[#3F4859]">
                    <section><h4 className="font-black text-[#141A26] border-b border-[#4F868C]/10 pb-2 mb-4 flex items-center gap-2 font-mono uppercase tracking-widest text-sm"><span className="bg-[#F2B705] text-[#141A26] w-6 h-6 rounded-full flex items-center justify-center text-[10px]">01</span> Plant Events</h4><p className="text-[13px] leading-relaxed">Click "Add Event" to schedule production shifts, maintenance, logistics, or audits. Colors automatically match category.</p></section>
                    <section><h4 className="font-black text-[#141A26] border-b border-[#4F868C]/10 pb-2 mb-4 flex items-center gap-2 font-mono uppercase tracking-widest text-sm"><span className="bg-[#D91604] text-white w-6 h-6 rounded-full flex items-center justify-center text-[10px]">02</span> Public Holidays</h4><p className="text-[13px] leading-relaxed">Use "Add Holiday" to register plant shutdowns or national holidays. Represents with a <b>Palm Tree</b> icon.</p></section>
                </div>
